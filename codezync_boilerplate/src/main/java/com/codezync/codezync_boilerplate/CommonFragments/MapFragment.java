package com.codezync.codezync_boilerplate.CommonFragments;
...
import com.codezync.codezync_boilerplate.GoogleLocation.FetchURL;
import com.codezync.codezync_boilerplate.GoogleLocation.MapAnimator;
import com.codezync.codezync_boilerplate.Listeners.MapViewListener;
...
import com.codezync.codezync_boilerplate.Utility.MapUtility;
import com.codezync.codezync_boilerplate.Utility.StringUtility;
...
//        mapStyleOptions = MapUtility.getMapStyle(getActivity(),);
        ...
        if (getActivity() != null) {
            ...
            if (mapFragment != null) {
                LogUtil.debug(TAG, "mapfragment not null");
                mapFragment.getMapAsync(this);
            } else {
                LogUtil.debug(TAG, "map fragment null");
            }
        }
    }
...
//                mMap.addMarker(markerOptions2);
    ...
    public void onMapReady(GoogleMap googleMap) {
        if (ActivityCompat.checkSelfPermission(getContext(), android.Manifest.permission.ACCESS_FINE_LOCATION) != PackageManager.PERMISSION_GRANTED && ActivityCompat.checkSelfPermission(getContext(),
            ...
            LogUtil.debug(TAG, "onMapReady > permission denied");
            return;
        } else {
            LogUtil.debug(TAG, "onMapReady > permission granted");
        }

        LogUtil.debug(TAG, "onMapReady");
        mMap = googleMap;
...
//    }
    ...
    public void drawPath(LatLng startLocation, LatLng endLocation, List<LatLng> wayPoint) {
        ...
        if (checkMapIsReady()) {
            ...
            LogUtil.debug(TAG, "WayPoint Req = " + url);
            new FetchURL(this).execute(url, "walking");
        }
...
//        }, 1000);
    ...
    public void moveMarkerWithCameraUpdate(Context context, Location newLocation, Marker marker) {
        LogUtil.debug(TAG, "drawPathForTracking moveMarkerWithCameraUpdate :" + new Gson().toJson(newLocation));
        new Handler().postDelayed(new Runnable() {
            @Override
            public void run() {
...
//    }
            ...
            valueAnimator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
                ...
                public void onAnimationUpdate(ValueAnimator animation) {
                    try {
                        ...
                        LogUtil.debug(TAG, "MOVE ANIMATION ERROR :" + ex.getLocalizedMessage());
                        //I don't care atm..
                    }
                }
            });
    ...
    private void animateMarkerNew(final LatLng destination, final Marker marker, boolean withCameraUpdate) {
        ...
        LogUtil.debug(TAG, "MOVING MARKER IS MARKER NULL : " + (marker == null));
        ...
        if (marker != null) {
            ...
            LogUtil.debug(TAG, "MOVING MARKER Start : " + marker.getPosition().toString());
            LogUtil.debug(TAG, "MOVING MARKER End : " + endPosition.toString());
            ...
            valueAnimator.setDuration(2000); // duration 3 second
            valueAnimator.setInterpolator(new LinearInterpolator());
            LogUtil.debug(TAG, "MOVING MARKER");
            valueAnimator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
                @Override
                public void onAnimationUpdate(ValueAnimator animation) {
                    try {
...
//                                    .build()));
                        ...
                        LogUtil.debug(TAG, "MOVING MARKER DONE");
                        LogUtil.debug(TAG, "MOVE ANIMATION ERROR :" + ex.getLocalizedMessage());
                        //I don't care atm..
                    }
                }
